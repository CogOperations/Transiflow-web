...
interface CardProps {
  variant?: "primary" | "secondary" | "tertiary" | "dashed" | "benefit";
  className?: string;
  icon?: React.ReactNode; // icon element
  title?: string; // heading
  description?: string; // paragraph
  bodyFontSize?: string; // font size for description
  bodyColor?: string; // color for description
  color?: string;
  paperColor?: "blue" | "green" | "red" | "purple" | "indigo";
}
...
const Card = ({
  ...
  const iconColor = colorClasses[color as keyof typeof colorClasses];

  const bgColorClasses: Record<NonNullable<CardProps["paperColor"]>, string> = {
    blue: "bg-blue-50",
    green: "bg-green-50",
    red: "bg-red-50",
    purple: "bg-purple-50",
      ...
      <div className={`${baseClasses} ${className}`}>
        ...
        {description && (
          <p
            ...
            style={{
              color: bodyColor ? bodyColor.replace("text-", "") : undefined,
            }}
          >
            {description}
          </p>
      ...
      {icon && (
        <motion.div
          ...
          className={`mb-3 ${iconColor} w-fit ${
            variant === "benefit" ? "rounded-full p-6" : "rounded-lg p-3 "
          }`}
        >
          {icon}
        </motion.div>
